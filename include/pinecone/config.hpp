...
constexpr auto kDefaultNameConvention{
    std::string_view{"abcdefghijklmnopqrstuvwxyz._0123456789"}};
...
class ConfigVarBase {
 public:
  ...
  explicit ConfigVarBase(std::string_view name,
  ...
  [[nodiscard]] virtual auto GetTypeName() const noexcept
      -> std::string_view = 0;
 ...
 public:
  ...
  [[nodiscard]] auto ToYaml() const noexcept -> std::string override {
    try {
      ...
      ELOGE << "ConfigVar::toString exception " << e.what()
  ...
  auto FromYaml(std::string_view yaml_str) -> void override {
    try {
      ...
      ELOGE << "ConfigVar::fromString exception " << e.what()
            << " convert: string to" << GetTypeName() << " name=" << name_
            << " - " << yaml_str;
    }
  }
...
class Config {
 public:
  ...
  template <class T>
  static auto Lookup(std::string_view name, const T &default_value,
    ...
    if (iter != GetDatas().end()) {
      ...
      ELOGE << "Lookup name=" << name << " exists but type not "
            << pinecone::util::TypeToName<T>()
  ...
  static auto LoadFromConfDir(
      std::string_view path = GetConfigDefaultRootPath()) -> void;
